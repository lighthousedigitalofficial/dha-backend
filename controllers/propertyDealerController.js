...
import { checkReferenceId } from "../models/propertyDealerModel.js";
...
export const createPropertyDealer = catchAsync(async (req, res, next) => {
  const { agency, slug, fullName, address, phone, affiliateId } = req.body;

  // Validate required fields
  if (!agency || !fullName || !affiliateId) {
    return next(new AppError("Please provide all required fields (agency, fullName, affiliateId).", 400));
  }

  // Check if affiliateId is valid
  const isAffiliateValid = await checkReferenceId('Affiliates', affiliateId);
  if (!isAffiliateValid) {
